...
%                 Rangedata_even(num_crop + 1:fft_Rang - num_crop,:,:));
            R_data = cat(2, Rangedata_odd, Rangedata_even);
            R_data = R_data/1e+04;
            Angdata = fft_angle(R_data,fft_Ang,Is_Windowed);
   
            Rcs_weight = repmat(rng_grid.^4,[1,128]);
            Rcs = Rcs_weight.*(sum(abs(Angdata).^2, 3)/255);
            max(Rcs(4:131,:), [], [1,2])
%             min_r = [min_r, min(sum(abs(R_data),3),[],[1,2])];
%             max_r = [max_r, max(sum(abs(R_data(4:131,:)),3),[],[1,2])];
%             figure(); 
%             surf(agl_grid, rng_grid(10:124),sum(Rcs(10:124,:,:),3)/255)
%             xlabel('angle')
%             ylabel('range')
%             axis([-90,90, 2,28])
%             colorbar
